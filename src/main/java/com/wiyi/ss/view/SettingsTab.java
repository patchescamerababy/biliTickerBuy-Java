...
import com.wiyi.ss.util.CookieManager;
            ...
            if (selectedFile != null) {
                try {
                    ...
                    if (parsed instanceof org.json.JSONArray) {
                        ...
                        if (cookies == null) {
                            usernameField.setText("配置文件中未找到cookies字段");
                            return;
                        }
                    } else {
                        usernameField.setText("无效的配置文件格式");
                        return;
                    }
                    ...
                    cookieFileField.setText(selectedFile.getAbsolutePath());
                    ...
                    usernameField.setText("导入失败: " + ex.getMessage());
                    ex.printStackTrace();
                }
            }
        });
    ...
    private static void loadCookiesAndUpdateUI() {
        ...
        if (cookieFile.exists()) {
            try {
                ...
                String username = biliRequest.getUserNickname();
                usernameField.setText(username);
                cookieFileField.setText(cookieFile.getAbsolutePath());
            } catch (Exception e) {
                e.printStackTrace();
                usernameField.setText("加载Cookie失败: " + e.getMessage());
            }
        }
    }
