...
public class GoTab {
    ...
    private static void startTicketBuying() {
        ...
        for (File configFile : uploadedFiles) {
            ...
            taskThread.start();
            runningTasks.add(taskThread); // Add thread to the list
            
            stopButton.setOnAction(e -> {
                taskThread.interrupt();
                runningTasks.remove(taskThread); // Remove thread from list on individual stop
                logArea.appendText("停止抢票任务: " + configFile.getName() + "\n");
            });

            // Also remove the thread from the list when the task window is closed
