...
public class GoTab {
    ...
    public static VBox create() {
        ...
        testProxyButton.setOnAction(e -> {
            ...
            // 新增：捕获CookieManager相关异常，避免无响应
            try {
                final com.example.util.CookieManager cookieManager = com.example.util.CookieManager.loadOrLoginIfNeeded(false);
                ...
                javafx.application.Platform.runLater(() -> {
                    showAlert("代理测试失败", "加载Cookie或登录流程异常：\n" + ex.getMessage());
                });
            }
        });
        ...
        TextField barkField = new TextField();
        barkField.setId("barkField");
        barkField.setPromptText("Bark的Token或完整URL（如：https://api.day.app/your_key/）");
        TextField ntfyField = new TextField();
        ntfyField.setId("ntfyField");
        ntfyField.setPromptText("Ntfy服务器URL");
        TextField ntfyUsernameField = new TextField();
        ntfyUsernameField.setId("ntfyUsernameField");
        ntfyUsernameField.setPromptText("Ntfy用户名（可选）");
        TextField ntfyPasswordField = new TextField();
        ntfyPasswordField.setId("ntfyPasswordField");
        ntfyPasswordField.setPromptText("Ntfy密码（可选）");
        notifyContent.getChildren().addAll(
            new Label("Server酱:"), serverChanField,
            new Label("PushPlus:"), pushPlusField,
            new Label("Bark:"), barkField,
            new Label("Ntfy服务器:"), ntfyField,
            new Label("Ntfy用户名:"), ntfyUsernameField,
            new Label("Ntfy密码:"), ntfyPasswordField
        );
    ...
    private static void startTicketBuying() {
        ...
        for (File configFile : uploadedFiles) {
            ...
            try {
                ...
                String barkToken = ((TextField) fileContentArea.getScene().lookup("#barkField")).getText();
                String ntfyUrl = ((TextField) fileContentArea.getScene().lookup("#ntfyField")).getText();
                String ntfyUsername = ((TextField) fileContentArea.getScene().lookup("#ntfyUsernameField")).getText();
                String ntfyPassword = ((TextField) fileContentArea.getScene().lookup("#ntfyPasswordField")).getText();
    ...
    public static void showAlert(String title, String message) {
        if(System.getProperty("os.name").toLowerCase().contains("win")){
            displayMessageBox(null, message, title, 0);
        }else {
            ...
            alertLayout.getChildren().addAll(
                    new Label(message),
                    new Button("确定") {{
                        setOnAction(e -> alertStage.close());
                    }}
            );
    ...
    public interface User32 extends StdCallLibrary {
        ...
        int MessageBoxW(WinDef.HWND hWnd, String lpText, String lpCaption, int uType);
    }
    public static int displayMessageBox(WinDef.HWND hWnd, String text, String caption, int type) {
        return User32.INSTANCE.MessageBoxW(hWnd, text, caption, type);
    }
}
