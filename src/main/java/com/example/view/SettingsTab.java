...
// 让文本自动换行并用计算高度
        ...
        warningLabel.setMinHeight(Region.USE_PREF_SIZE);
        warningLabel.setPrefHeight(120); // 适度增大默认高度
        warningLabel.setMaxWidth(Double.MAX_VALUE);
        warningLabel.prefWidthProperty().bind(layout.widthProperty().subtract(20)); // 扣掉左右 10px 内边距
        ...
        // 将网址输入框放在浅蓝色框内顶部
        ticketSection.getChildren().add(urlInputBox);
        ...
        detailsGrid.add(new Label("日期:"), 0, 1);
        ...
        ticketSection.getChildren().add(ticketDetailsPane);

        getInfoButton.setOnAction(e -> {
            String url = ticketUrlField.getText();
            if (url != null && url.startsWith("https://show.bilibili.com/platform/detail.html?id=")) {
                ...
                if (System.getProperty("os.name").toLowerCase().contains("win")) {
                    int returnCode = GoTab.displayMessageBox(null, "请确认网址有效", "警告", 4 | 48);
                    while (returnCode == 7) {
                        returnCode = GoTab.displayMessageBox(null, "请按确认", "警告", 4 | 48);
                    }
                }
        ...
        captchaTestSection.setVisible(false);
        captchaTestSection.setManaged(false); // 设置为不参与布局计算
        ...
        layout.getChildren().addAll(
                warningLabel,   // ① 警示信息 —— 第一位
                loginSection,   // ② 登录区域
                phoneAccordion, // ③ 手机号（可选）
                ticketSection   // ④ 票务区域
        );
