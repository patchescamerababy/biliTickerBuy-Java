...
public class ProxyTester {
     ...
     * @return 测试结果
     */
    public ProxyTestResult testSingleProxy(String proxyStr, String cookie) {
    ...
    private OkHttpClient buildClient(String proxyStr) {
        // 处理代理设置
        if (proxyStr != null && !"none".equalsIgnoreCase(proxyStr) && !"direct".equalsIgnoreCase(proxyStr) && validateProxyFormat(proxyStr)) {
            return OkHttpClientUtil.buildClient(proxyStr);
        }
        return OkHttpClientUtil.buildClient();
    }
    /**
     * 验证代理格式是否正确
     *
     ...
     * @return 测试结果列表
     */
    public List<ProxyTestResult> testProxyList(String proxyString, int maxWorkers, String cookie) {
        ...
        for (String proxy : proxyList) {
            Future<ProxyTestResult> future = executor.submit(() -> {
                ProxyTestResult result = testSingleProxy(proxy, cookie);
     ...
     * @return 测试结果列表
     */
    public List<ProxyTestResult> testProxyList(String proxyString, String cookie) {
        return testProxyList(proxyString, 5, cookie);
    }
     ...
     * @return 格式化的测试结果
     */
    public static String testProxyConnectivity(String proxyString, int timeout, String cookie) {
        ProxyTester tester = new ProxyTester(timeout);
        List<ProxyTestResult> results = tester.testProxyList(proxyString, 5, cookie);
     ...
     * @return 格式化的测试结果
     */
    public static String testProxyConnectivity(String proxyString, String cookie) {
        return testProxyConnectivity(proxyString, 10, cookie);
    }
}
